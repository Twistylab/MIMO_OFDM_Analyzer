...
from DFE_class import BERAnalyzerWithDFE


def run_single_method(ber_key, method, **kwargs):
    analyzer = BERAnalyzerWithDFE(**{method: True}, **kwargs)
    analyzer.run_full_analysis()

    return ber_key, analyzer.ber

def compute_ber_multiprocessing(use_dfe_hard: bool = False, use_dfe_fec_hard: bool = False, use_dfe_fec_soft: bool = False,
                                modulation_size: int = 16, bits_number: int = 10000, noise_variance: float = None, impulse_response_length: int = 10, delay: int = 10) -> dict:

    methods = {
        'ber_dfe_hard': {'use_dfe_hard': use_dfe_hard},
        'ber_dfe_fec_hard': {'use_dfe_fec_hard': use_dfe_fec_hard},
        'ber_dfe_fec_soft': {'use_dfe_fec_soft': use_dfe_fec_soft},
    }
